...
const skillCategories = [
  ...
  },
  {
    ...
    skills: [
      ...
      { name: "Django",  icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/django/django-plain.svg",  style: { filter: 'invert(1) brightness(2)' } },
...
export default function Skills() {
  return (
    <section id="skills" className="section-container" aria-label="System Capabilities Section" style={{ paddingTop: '8rem', paddingBottom: '4rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', maxWidth: '1100px', margin: '0 auto' }}>
        {skillCategories.map((category, i) => (
          ...
          >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(80px, 1fr))', gap: '2rem 1rem' }}>
              {category.skills.map((skill, j) => (
                <motion.div
                  key={j}
                  whileHover={{ scale: 1.15, rotateX: 12, rotateY: 12, z: 20 }}
                ...
                >
                  <motion.img
                    ...
                    alt={skill.name}
                    loading="lazy"
                    style={{ width: 'clamp(30px, 8vw, 40px)', height: 'clamp(30px, 8vw, 40px)', objectFit: 'contain', ...(skill.style || {}) }}
                  />
                  <span style={{ color: 'var(--text-main)', fontSize: '0.75rem', fontWeight: 600, textAlign: 'center', fontFamily: 'var(--font-heading)', letterSpacing: '1px' }}>
                    {skill.name}
                  </span>
                </motion.div>
              ))}
